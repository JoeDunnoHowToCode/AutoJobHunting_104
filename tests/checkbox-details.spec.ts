...
describe('collectCheckboxDetails — label 來源', () => {
  it('以 for 屬性關聯的 label', () => {
    ...
    expect(collectCheckboxDetails()[0].label).toBe('我同意個資使用條款');
  });

  it('包裹式 label', () => {
    render('<label><input type="checkbox">訂閱職缺電子報</label>');
    expect(collectCheckboxDetails()[0].label).toBe('訂閱職缺電子報');
  });

  it('aria-label', () => {
    render('<input type="checkbox" aria-label="開放企業查看履歷">');
    expect(collectCheckboxDetails()[0].label).toBe('開放企業查看履歷');
  });

  it('for 屬性優先於 aria-label', () => {
    render('<label for="c">來自 label</label><input id="c" type="checkbox" aria-label="來自 aria">');
    expect(collectCheckboxDetails()[0].label).toBe('來自 label');
  });

  it('完全無 label 時回空字串而非 undefined', () => {
    render('<input type="checkbox">');
    expect(collectCheckboxDetails()[0].label).toBe('');
  });

  it('label 文字前後空白被修掉', () => {
    render('<label for="c">   我同意   </label><input id="c" type="checkbox">');
    expect(collectCheckboxDetails()[0].label).toBe('我同意');
  });
});
...
describe('collectCheckboxDetails — 狀態欄位', () => {
  it('回報勾選狀態', () => {
    ...
    const details = collectCheckboxDetails();
  ...
  it('回報 name 供辨識', () => {
    ...
    expect(collectCheckboxDetails()[0].name).toBe('privacyAgree');
  });
});
...
describe('collectCheckboxDetails — 範圍', () => {
  it('display:none 的 checkbox 不列入', () => {
    ...
    expect(collectCheckboxDetails()).toHaveLength(1);
  });
  ...
  it('沒有任何 checkbox 時回空陣列', () => {
    render('<div>沒有表單</div>');
    expect(collectCheckboxDetails()).toEqual([]);
  });
  ...
  it('多個 checkbox 依文件順序回傳', () => {
    ...
    expect(collectCheckboxDetails().map(d => d.label)).toEqual(['第一', '第二']);
  });
});
...
describe('collectCheckboxDetails — 純診斷，不得改動 DOM', () => {
  ...
  it('回傳結構不含任何分類判斷欄位', () => {
    ...
    const detail = collectCheckboxDetails()[0];
